...
import com.example.android.camera2basic.utils.CameraHelper;
...
public class CameraGLView extends GLSurfaceView {
    ...
    protected ImageView cameraSwitcher;
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        if (!mRenderer.showFilters) return super.dispatchTouchEvent(event);
    ...
    public void touched (){
        if(touchedCount==0){ touchedCount++; mRenderer.touched("bw");}
        else if(touchedCount==1){touchedCount++; mRenderer.touched("negative");}
        else if(touchedCount==2){touchedCount++; mRenderer.touched("sepia");}
        else if(touchedCount==3){touchedCount=0; mRenderer.touched("none");}
    ...
    private static final class CameraSurfaceRenderer
        ...
        public CameraSurfaceRenderer(final CameraGLView parent) {
            Log.d(TAG, "BitmapSurfaceRenderer:");
        ...
        public void onTouched(int x, int y) {
            ...
            if (parent == null || parent.fragRef.get() == null) return;
